---
title: "Animations in R: Cloudiness in Germany"
author: "Aleksei Prishchepo"
date: "2025-07-07"
description: "Creating an animated visualization of cloudiness in Germany using R."
format:
  html:
    toc: true
...
knitr::opts_knit$set(root.dir = "~/Animations-in-R/Deutsche-Wetterdienst")
```
...
if (length(files) == 0) {
  for (link in links$links) {
    download.file(paste0(url, link), here(
      "Deutsche-Wetterdienst", "data", link
    ), mode = "wb")
...
con <- dbConnect(duckdb(),
  dbdir = here("Deutsche-Wetterdienst", "db", "weather.duckdb")
)
...
if (!("cloudiness" %in% tables)) {
  ...
  con <- dbConnect(duckdb(),
    dbdir = here(
      "Deutsche-Wetterdienst", "db", "weather.duckdb"
    )
  )
...
observations[MESS_DATUM > "2024-01-01" & STATIONS_ID == 433] |>
  ...
  # geom_line() +
  geom_line(aes(y = rollmean(cloud_coverage, 7, na.pad = TRUE)), color = "blue") +
  labs(title = "Cloud Coverage", x = NULL, y = "Cloud Coverage") +
  theme_minimal()
```
...
of the stations in the observations data.
...
observations[, cloud_coverage_r7 := rollmean(cloud_coverage, 7, na.pad = TRUE),
  by = MESS_DATUM
]
...
png_files <- list.files(
  here("Deutsche-Wetterdienst", "figures"),
...
gif_file <- here(
  "Deutsche-Wetterdienst", "animation-cloudiness.gif"
)
