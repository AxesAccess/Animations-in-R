...
knitr::opts_knit$set(root.dir = "~/Animations-in-R/Deutscher-Wetterdienst")
```
...
of the stations in the observations data. Calculate mean for each H3 address.

```{r join-stations-observations}
observations <- observations[stations, on = "STATIONS_ID"]
observations <- observations[, .(coud_coverage = mean(cloud_coverage, rm = T)),
  by = h3_address
]
```
...
gif_file <- here(
  "Deutscher-Wetterdienst", "animation-cloudiness.gif"
)
